...
namespace MJGame
{
    ...
    [System.Serializable]
    public class SerializableDictionaryExtension<TKey, TValue> : IEnumerable, IDictionary, IDictionary<TKey, TValue>, IReadOnlyDictionary<TKey, TValue>, IDeserializationCallback
    {
        [SerializeField, HideField] bool info;


        [SerializeField, HideField]
        protected SerializableSetExtension<TKey> keys;
        ...
        public SerializableDictionaryExtension()
        {
            this.keys = new();
            this.values = new();
        }
        public SerializableDictionaryExtension(IDictionary<TKey, TValue> dictionary)
        {
            if (dictionary == null)
                throw new NullReferenceException("Dictionary is null");
            keys = new SerializableSetExtension<TKey>(dictionary.Select(_ => _.Key));
            values = dictionary.Select(_ => _.Value).ToList();
        }
        public SerializableDictionaryExtension(IEnumerable<KeyValuePair<TKey, TValue>> collection)
        {
            if (collection == null)
                throw new NullReferenceException("IEnumerable is null");
            keys = new SerializableSetExtension<TKey>(collection.Select(_ => _.Key));
            values = collection.Select(_ => _.Value).ToList();
        }
        protected SerializableDictionaryExtension(SerializableSetExtension<TKey> keys, List<TValue> values)
        {
            if (keys == null)
        ...
        public static explicit operator (List<TKey> keys, List<TValue> values)(SerializableDictionaryExtension<TKey, TValue> d)
            => ((List<TKey>)d.keys, d.values);
        public static explicit operator List<(TKey, TValue)>(SerializableDictionaryExtension<TKey, TValue> d)
        {
        ...
        public static explicit operator Dictionary<TKey, TValue>(SerializableDictionaryExtension<TKey, TValue> d)
        {
        ...
        protected class DictionaryEnumerator : IDictionaryEnumerator
        {
            ...
            public DictionaryEnumerator(SerializableDictionaryExtension<TKey, TValue> sd)
            {
        ...
        protected virtual void Internal_OnDeserialization(object sender)
        {
            if (keys.Count != values.Count)
            {
                if (keys.Count > values.Count)
                {
                    ...
                    Debug.LogError(nameof(SerializableDictionaryExtension<TKey, TValue>) + ": Deserialized 'key's amount was greater than 'value's amount. " +
                ...
                else
                {
                    ...
                    Debug.LogError(nameof(SerializableDictionaryExtension<TKey, TValue>) + ": Deserialized 'value's amount was greater than 'key' amount. " +
