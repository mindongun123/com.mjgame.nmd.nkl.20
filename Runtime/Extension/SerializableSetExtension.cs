...
namespace MJGame
{
    ...
    [System.Serializable]
    public class SerializableSetExtension<T> : ICollection, ICollection<T>, IEnumerable, IEnumerable<T>, IDeserializationCallback
    {
        ...
        public SerializableSetExtension()
        {
            values = new();
        }
        public SerializableSetExtension(IEnumerable<T> collection)
        {
            if (collection == null)
        ...
        public static explicit operator List<T>(SerializableSetExtension<T> s)
            => s.values;
