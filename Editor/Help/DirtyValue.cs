...
public class DirtyValue
{
    ...
    void FollowPath(string propertyPath, BindingFlags bindingAttr = PropertyValues.defaultBindingFlags, bool forceFind = false)
    {
        ...
        if (properties.Length > 0)
        {
            ...
            {
                if (properties[i] != "Array") // last was no array
                {
                    nested.level = 0;
                    nested.indices = new();

                    fieldInfo = Type.GetFieldWholeInheritance(properties[i], bindingAttr);
                    Type = fieldInfo.FieldType;
                ...
                {
                    if (++i < properties.Length) //wenn es ein element der liste ist : MyClassName.myArrayName.Array.data[i]
                    {
                        if (IsIList())
                        {
                            int.TryParse(properties[i][5..^1], out int dataIndex); //Its naming is always "data[i]"
